"""
OrQuanta Agentic v1.0 — One-Click Startup Script
...
{C.BOLD}{C.MAGENTA}
  ╔══════════════════════════════════════════════════╗
  ║          OrQuanta Agentic v1.0 Platform             ║
  ║     AI-Native GPU Cloud Infrastructure           ║
  ╚══════════════════════════════════════════════════╝
{C.R}""")
...
# ─── Default .env content for local dev ──────────────────────────────────────
DEFAULT_ENV = """# OrQuanta Agentic v1.0 — Local Development Environment
# Auto-generated by start_orquanta.py — edit as needed
...
# ── Cloud Providers (optional — simulated without keys) ────────
...
def check_nodejs(frontend_only: bool = True):
    ...
    if not node or not npm:
        warn("Node.js / npm not found — frontend dev server won't start")
...
def setup_env():
    ...
    if not ENV_FILE.exists():
        ...
        info("⚡ Edit v4/.env to add real API keys for cloud providers")
    else:
...
def wait_for_service(url: str, service_name: str, timeout_s: int = 120) -> bool:
    ...
    print(f"  {C.CYAN}→{C.R} Waiting for {service_name}", end="", flush=True)
...
def start_api_direct():
    ...
    info("Starting API server (uvicorn --reload)…")
...
def main():
    ...
    if not checks_ok:
        warn("Some prerequisites failed — continuing anyway (may encounter errors)")
    ...
    if args.no_docker:
        # ── Step 3: Direct mode (no Docker) ─────────────────────────
        step("3/7 — Starting API Server Directly")
        api_proc = start_api_direct()
        procs.append(("API", api_proc))

        step("4/7 — Waiting for API…")
        ...
        step("3/7 — Starting Docker Infrastructure")
        compose_cmd = get_compose_cmd()
        start_services(compose_cmd, prod=args.prod)

        step("4/7 — Waiting for Services to be Healthy")
        db_ok = wait_for_service("http://localhost:8000/health", "API (Docker)", timeout_s=90)
        if not db_ok:
            warn("API health check did not pass — check logs: docker compose logs api")

        step("5/7 — Running Database Migrations")
        run_migrations()

    # ── Step 6: Frontend ─────────────────────────────────────────────
    step("6/7 — Starting Frontend Dev Server")
    ...
    if procs:
        info(f"Running {len(procs)} processes — Ctrl+C to stop all")
        try:
            while True:
                time.sleep(1)
