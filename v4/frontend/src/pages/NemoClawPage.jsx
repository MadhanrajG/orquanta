...
const STATUS_COLOR = { active: '#10B981', stopped: '#ff6b6b', unknown: '#94a3b8' }
const ACTION_COLOR = { halt: '#ff4444', throttle: '#F59E0B', warn: '#0091FF', prewarm: '#7B2FFF' }

function StatCard({ icon: Icon, label, value, sub, color = '#0091FF', glow = false }) {
    return (
        <div className="glass-card p-5 flex items-start gap-4" style={{
            ...
            <div>
                ...
                <p className="text-gray-900 text-xl font-bold font-mono">{value}</p>
...
function TraceStep({ step, idx }) {
    const phaseColors = {
        CONTEXT: '#7B2FFF', ACT: '#0091FF', SELF_EVAL: '#10B981',
        ERROR: '#ff4444', OBSERVE: '#F59E0B',
    }
    ...
    return (
        <div className="flex gap-3 items-start py-2 border-b border-white/[0.04]">
            ...
            <div className="flex-1 min-w-0">
                <p className="text-xs text-slate-600 truncate">{step.action}</p>
...
export default function NemoClawPage() {
    ...
    const fetchAll = useCallback(async () => {
        try {
            ...
            setError('')
        } catch (e) { setError('NemoClaw API unreachable â€” start the backend server.') }
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex items-start justify-between">
                <div>
                    <div className="flex items-center gap-3 mb-1">
                        ...
                        <div>
                            <h1 className="text-2xl font-bold text-gray-900">NemoClaw Engine</h1>
                            <p className="text-xs text-slate-500">OpenClaw Multi-Agent Cognitive Layer Â· v1.0.0</p>
                        </div>
                    </div>
                    <p className="text-sm text-slate-400 max-w-xl">
                        ContextGraph Â· AdaptiveReAct Â· PredictivePrefetch Â· CostWatcher â€” institutional memory for agentic GPU orchestration.
                    </p>
                </div>
            ...
            {tab === 'overview' && (
                <div className="space-y-5">
                    <div className="grid grid-cols-2 gap-4" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))' }}>
                        <StatCard icon={Database} label="Context Nodes" value={status?.context_nodes ?? 'â€”'} sub="Persistent knowledge graph" color="#7B2FFF" glow />
                        <StatCard icon={Eye} label="Active Traces" value={status?.active_traces ?? 'â€”'} sub="AdaptiveReAct traces" color="#0091FF" />
                        <StatCard icon={Zap} label="Goals Processed" value={status?.total_goals_processed ?? 'â€”'} sub="Through NemoClaw" color="#10B981" />
                        <StatCard icon={CheckCircle} label="Avg Confidence" value={status ? `${(status.avg_confidence * 100).toFixed(0)}%` : 'â€”'} sub="Self-eval score" color="#F59E0B" />
                        <StatCard icon={Shield} label="Cost Brakes" value={status?.cost_brakes_fired ?? 'â€”'} sub="Budget enforcements" color="#ff6b6b" />
                        <StatCard icon={Server} label="Prefetch Recs" value={status?.prefetch_recommendations?.length ?? 'â€”'} sub="GPU pre-warm alerts" color="#A78BFA" />
                    </div>
                                ...
                                {status.prefetch_recommendations.map(rec => (
                                    <div key={rec.rec_id} className="flex items-center gap-3 p-3 rounded-lg" style={{ background: 'rgba(167,139,250,0.05)', border: '1px solid rgba(167,139,250,0.15)' }}>
                                        <div className="flex-1">
                                            <p className="text-sm text-gray-900 font-medium">{rec.predicted_gpu} via {rec.predicted_provider}</p>
            ...
            {/* â”€â”€ Run Goal Tab â”€â”€ */}
            {tab === 'run' && (
                <div className="glass-card p-6 space-y-5">
                    <h3 className="text-gray-900 font-semibold">AdaptiveReAct Goal Execution</h3>
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-600 mb-1.5">Goal Description</label>
                        <textarea
                    ...
                    <div>
                        <label className="block text-sm font-medium text-slate-600 mb-1.5">Budget (USD): ${budgetUsd}</label>
                    ...
                    <button className="btn-primary flex items-center gap-2" onClick={runGoal} disabled={running || !goalText.trim()}
                        style={{ background: 'linear-gradient(135deg, #7B2FFF, #0091FF)' }}>
                    ...
                    {runResult && (
                        <div className="glass-card p-4 border border-violet-500/20 space-y-2">
                            {runResult.error ? (
                                <p className="text-red-600 text-sm">{runResult.error}</p>
                            ) : (
                                <>
                                    <div className="flex items-center gap-2 mb-2">
                                        <CheckCircle size={14} style={{ color: '#10B981' }} />
                                        <span className="text-sm font-medium text-gray-900">Goal Submitted via NemoClaw</span>
                                    </div>
                                    <p className="text-xs text-slate-400">Trace ID: <code className="text-violet-400">{runResult.trace_id}</code></p>
                                    <p className="text-xs text-slate-400">Confidence: <span className="text-gray-900 font-semibold">{((runResult.confidence || 0) * 100).toFixed(0)}%</span></p>
                                    <p className="text-xs text-slate-400">Context nodes used: <span className="text-gray-900">{runResult.past_context_used}</span></p>
                                    ...
                                    {runResult.prefetch_recommendations?.length > 0 && (
                                        <p className="text-xs text-violet-400">
                                            ðŸ’¡ Prefetch: Pre-warm {runResult.prefetch_recommendations[0].predicted_gpu} on {runResult.prefetch_recommendations[0].predicted_provider}
                                        </p>
                                    )}
                                </>
                            )}
            ...
            {tab === 'traces' && (
                <div className="grid gap-5" style={{ gridTemplateColumns: selectedTrace ? '1fr 1fr' : '1fr' }}>
                    <div className="glass-card p-5">
                        <h3 className="text-gray-900 font-semibold mb-3">Recent AdaptiveReAct Traces</h3>
                        {traces.length === 0 ? (
                            ...
                            <div className="space-y-2">
                                {traces.map(t => (
                                    <button key={t.trace_id} onClick={() => loadTrace(t.trace_id)}
                                        ...
                                        style={{ border: `1px solid ${selectedTrace?.trace_id === t.trace_id ? 'rgba(123,47,255,0.4)' : 'rgba(0,0,0,0.04)'}` }}>
                                        <div className="flex justify-between items-center">
                                            <code className="text-xs text-violet-400">{t.trace_id}</code>
                                            <span className="text-xs" style={{ color: t.status === 'completed' ? '#10B981' : '#F59E0B' }}>{t.status}</span>
                                        </div>
                                        <p className="text-xs text-slate-400 mt-1">{t.steps_count} steps Â· {(t.final_confidence * 100).toFixed(0)}% confidence</p>
                                    </button>
                                ))}
                            </div>
                        )}
                    ...
                    {selectedTrace && (
                        <div className="glass-card p-5">
                            <h3 className="text-gray-900 font-semibold mb-1">Trace Detail</h3>
                            <p className="text-xs text-slate-500 mb-3">
                                Confidence: <span className="text-gray-900">{(selectedTrace.final_confidence * 100).toFixed(0)}%</span>
                                {' Â· '}Replans: <span className="text-gray-900">{selectedTrace.replans_triggered}</span>
                            </p>
            ...
            {/* â”€â”€ ContextGraph Tab â”€â”€ */}
            {tab === 'context' && (
                <div className="space-y-4">
                    <div className="glass-card p-5">
                        <h3 className="text-gray-900 font-semibold mb-3">ContextGraph Statistics</h3>
                        {contextStats ? (
                            <div className="grid grid-cols-2 gap-4">
                                <div className="glass-card p-4 bg-white/[0.02]">
                                    <p className="text-xs text-slate-500 mb-1">Total Nodes</p>
                                    <p className="text-2xl font-bold text-gray-900 font-mono">{contextStats.total_nodes}</p>
                                </div>
                                <div className="glass-card p-4 bg-white/[0.02]">
                                    <p className="text-xs text-slate-500 mb-1">Unique Users</p>
                                    <p className="text-2xl font-bold text-gray-900 font-mono">{contextStats.total_users}</p>
                                </div>
                    ...
                    <div className="glass-card p-5 border border-violet-500/20">
                        <h3 className="text-gray-900 font-semibold mb-2 text-sm">About the ContextGraph</h3>
                        <p className="text-xs text-slate-400 leading-relaxed">
                            The NemoClaw ContextGraph stores every goal, decision, and outcome as a weighted semantic node.
                            It enables cross-session institutional memory â€” the more goals you run, the smarter OrQuanta gets
            ...
            {/* â”€â”€ CostWatcher Tab â”€â”€ */}
            {tab === 'cost' && (
                <div className="glass-card p-5">
                    <h3 className="text-gray-900 font-semibold mb-3">CostWatcher â€” Budget Enforcement Events</h3>
                    {brakes.length === 0 ? (
                        <div className="text-center py-8">
                            <TrendingDown size={32} className="mx-auto mb-2 opacity-20 text-gray-900" />
                        ...
                        <div className="space-y-2">
                            {brakes.map(b => (
                                <div key={b.brake_id} className="flex items-center gap-4 p-3 rounded-lg"
                                    ...
                                    <div className="flex-1">
                                        <p className="text-sm text-gray-900">${b.spent_usd} / ${b.budget_usd} ({b.pct_used}%)</p>
                                        <p className="text-xs text-slate-500">Switch to: {b.alternative} Â· Save ${b.potential_save_usd}</p>
                                    </div>
