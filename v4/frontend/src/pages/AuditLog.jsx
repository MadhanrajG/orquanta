...
export default function AuditLog() {
 ...
 fetch(`${API}/api/v1/audit?${qs}`, opts),
 fetch(`${API}/api/v1/audit/stats`, opts),
 ])
 if (auditRes.ok) { const d = await auditRes.json(); setEntries(d.entries || []); setTotal(d.total ?? 0) }
 if (statsRes.ok) setStats(await statsRes.json())
 } catch (e) {
 if (e.name !== 'AbortError') { /* keep stale data visible on transient errors */ }
 }
 ...
 const t = setInterval(() => fetchAudit(controller.signal), 10000)
 ...
 <button onClick={() => fetchAudit(new AbortController().signal)} className="ml-auto btn-ghost flex items-center gap-1.5 text-xs">
